...
# data.  The script was written in Python 2.7.11

# Last updated: 2017/10/30 12:20:31.
    ...
    if ("pdf" in driver):
        if os.path.exists(out_path):
            confirmation = q.Confirm(
                ...
                title="Confirmation", default=True, ok=False, cancel=False)
            if not confirmation:
                out_path = q.SetFile(directory=in_dir, filename='',
                                     overwrite=False, title="Save",
                                     wildcard='Output file (*.pdf)|*.pdf')

    if not os.path.exists(conf_path):
        conf_path = q.GetFile(directory=in_dir, filename='',
                              multiple=False,
...
def get_path_quickgui(wildcard):
    path = q.GetFile(directory='', filename='', multiple=False,
    ...
    for i in range(len(time)):
        if time[i]/age_unit % L == 0:
            ...
            ax[axn].annotate(
                ...
                xy=(Xconv[i], Yconv[i]),
                xytext=(3, -6), fontsize=legend_font_size-2,
...
def ConcLineTW(t):
    X = 1./(np.exp(l238U*t) - 1)
    Y = ((1./X + 1.)**(l235U/l238U) - 1.)*X/U85r
    return(X, Y)
...
def calc_age_7Pb_6Pb(age_unit, j, je, age_7Pb_6Pb, conf):
    ...
    cr = stats.norm.ppf(conf+(1-conf)/2.)
    for i in range(len(j)):
        age_7Pb_6Pb[i] = optimize.leastsq(
            func_age_7Pb_6Pb, age_unit, args=(j[i]))[0][0]
        age_7Pb_6Pb_upper[i] = optimize.leastsq(
            func_age_7Pb_6Pb, age_unit, args=(j[i]+je[i]*cr))[0][0]
        ...
        if (age_7Pb_6Pb[i] < 0.0): age_7Pb_6Pb[i] = 0.0
...
def myEllipse(i, x, y, sigma_x, sigma_y, cov_xy,
              conf='none', alpha='none',
              fc='none', edgecolor='none', edgewidth=0.5, linestyle='solid'):
    cov = ([sigma_x**2, cov_xy], [cov_xy, sigma_y**2])
    vals, vecs = eigsorted(cov)
    theta = np.degrees(np.arctan2(*vecs[:, 0][::-1]))
    if (vals[0] < 0.0) | (vals[1] < 0.0):
        print('!!! Unable to draw an error ellipse [Data# %d] !!!') % (i+1)
        width, height = [0., 0.]
    else:
        width, height = 2*np.sqrt(stats.chi2.ppf(conf, 2))*np.sqrt(vals)

    ell = Ellipse(xy=(x, y), width=width, height=height,
                  angle=theta, alpha=alpha, fc=fc,
                  ec=edgecolor, lw=edgewidth, ls=linestyle)
    return(ell)
...
def oneWM(X, s1, conf):
    ...
    sm = stats.norm.ppf(conf + (1 - conf)/2.)*np.sqrt(1./np.sum(s1**(-2)))
...
def twoWM(Xi, Yi, sXi, sYi, rhoXYi, conf):
    ...
    # eq(9)
    cov_xy_bar = -np.sum(o12)/(
        np.sum(o11)*np.sum(o22)-np.sum(o12)**2)
    return(x_bar, y_bar, MSWD,
           sigma_x_bar, sigma_y_bar, cov_xy_bar)
...
def ConcAgeConv(Xi, Yi, sigma_Xi, sigma_Yi,
                rhoXYi, Tinit=10.**6, conf=0.95):
    ...
    T_leastsq = optimize.leastsq(
        FitFuncConv, Tinit,
        args=(X_bar, Y_bar, sigma_X_bar, sigma_Y_bar, rho_XY_bar))[0][0]
    # eq(3)
    ...
    T_1sigma = np.sqrt(QQ)
    T_sigma = stats.norm.ppf(conf+(1-conf)/2.)*T_1sigma
    ...
    df_concordance = 1
    df_equivalence = 2*len(Xi)-1
    MSWD_concordance = S_bar/df_concordance
    MSWD_equivalence = S/df_equivalence
    MSWD_combined = (S_bar+S)/df_equivalence
    if (ca_mswd == 1):
        MSWD = MSWD_equivalence
        P_value = 1. - stats.chi2.cdf(MSWD,df_equivalence)
    elif (ca_mswd == 2):
        MSWD = MSWD_combined
        P_value = 1. - stats.chi2.cdf(MSWD,df_equivalence)
    else:
        MSWD = MSWD_concordance
        P_value = 1. - stats.chi2.cdf(MSWD,df_concordance)
    return(T_leastsq, T_sigma, MSWD, P_value)


# Tera-Wasserburg concordia curve (p. 668 in Ludwig, 1998)
def ConcAgeTW(Xi, Yi, sigma_Xi, sigma_Yi,
              rhoXYi, Tinit=10.**6, conf=0.95):
    x = U85r*Yi/Xi
    y = 1./Xi
    sigma_x = np.sqrt(
        ((sigma_Xi/Xi)**2+(sigma_Yi/Yi)**2 -
         2*(sigma_Xi/Xi)*(sigma_Yi/Yi)*rhoXYi))*x
    sigma_y = sigma_Xi/Xi*y
    rhoxy = ((sigma_Xi/Xi)**2-(sigma_Xi/Xi)*(sigma_Yi/Yi) *
             rhoXYi) / ((sigma_x/x)*(sigma_y/y))
...
def Fit_XYZ(b, x, y, wx, wy, r):
    # eq(2)
    z = wx*wy/(b**2*wy+wx-2.*b*r*np.sqrt(wx*wy))
    # eq(4)
...
def SlopeIntercept(x, y, sigma_x, sigma_y, rho_xy, case):
    ...
    if (case == 1):
        bi1 = optimize.leastsq(
            FitFuncSI, init_b,
            args=(x, y, wx, wy, rho_xy, case))[0][0]
        X_bar1, Y_bar1, Z1, S1 = Fit_XYZ(bi1, x, y, wx, wy, rho_xy)
    else:
        bi2 = optimize.leastsq(
            FitFuncSI, init_b,
            args=(x, y, wx, wy, rho_xy, case))[0][0]
    ...
    sigma_bi = np.sqrt(1./np.sum(Z*(x-X_bar)**2))
    # sigma_ai = np.sqrt(np.sum(x**2*Z)/(np.sum(Z)*np.sum(Z*(x-X_bar)**2)))
    sigma_ai = np.sqrt(1./np.sum(Z) + X_bar**2*sigma_bi**2)     # York2004ajp
...
def SIsigma(x, x_bar, y_bar, b,
            sigma_a, sigma_b, conf=0.95):
    # eq(29)
    sigma_a2 = stats.norm.ppf(conf+(1-conf)/2.)*sigma_a
    sigma_b2 = stats.norm.ppf(conf+(1-conf)/2.)*sigma_b
    sigma2 = np.sqrt(sigma_a2**2.+sigma_b2**2.*x*(x-2.*x_bar))
    return(sigma2)


def SIsigma2(x, x_bar, y_bar, b,
             sigma_a, sigma_b, conf=0.95):
    # eq(30)
    dtheta = sigma_b*np.cos(np.arctan(b))**2.
    b2 = (np.tan(np.arctan(b)+dtheta) +
          np.tan(np.arctan(b)-dtheta))/2.
    a2 = y_bar-b2*x_bar
    sigma_b2 = (np.tan(np.arctan(b)+dtheta) -
                np.tan(np.arctan(b)-dtheta))/2.
    sigma_a2 = sigma_a+(sigma_b2-sigma_b)*x_bar
    sigma_b2 = stats.norm.ppf(conf+(1-conf)/2.)*sigma_b2
    sigma_a2 = stats.norm.ppf(conf+(1-conf)/2.)*sigma_a2
    # eq(31)
    sigma2 = np.sqrt(sigma_a2**2+sigma_b2**2.*x*(x-2.*x_bar))
...
def FitFuncSIageConv(t, a, b, sigma_a, sigma_b,
                     x_bar, y_bar, conf, case):
    x1 = np.exp(l235U*t)-1.
    y1 = np.exp(l238U*t)-1.
...
def SIageConv(a, b, sigma_a, sigma_b,
              x_bar, y_bar, init_t=10**6, conf=0.95):
    T = optimize.leastsq(
        FitFuncSIageConv, init_t,
        args=(a, b, sigma_a, sigma_b,
              x_bar, y_bar, conf, 0))[0][0]
    Tmin = optimize.leastsq(
        FitFuncSIageConv, init_t,
        args=(a, b, sigma_a, sigma_b,
              x_bar, y_bar, conf, 1))[0][0]
    Tmax = optimize.leastsq(
        FitFuncSIageConv, init_t,
        args=(a, b, sigma_a, sigma_b,
              x_bar, y_bar, conf, 2))[0][0]
    if Tmax < Tmin:
        Tmax, Tmin = Tmin, Tmax
    return(T, Tmin, Tmax)
...
def FitFuncSIageTW(t, a, b, sigma_a, sigma_b,
                   x_bar, y_bar, conf, cs):
    x1 = 1./(np.exp(l238U*t)-1)
    y1 = 1./U85r*(np.exp(l235U*t)-1)/(np.exp(l238U*t)-1)
    sigma = SIsigma(x1, x_bar, y_bar, b, sigma_a, sigma_b, conf=conf)
    if cs == 1:
        y2 = b*x1+a - sigma
    elif cs == 2:
...
def SIageTW(a, b, sigma_a, sigma_b,
            x_bar, y_bar, init_t=10**6, conf=0.95):
    T = optimize.leastsq(
        FitFuncSIageTW, init_t,
        args=(a, b, sigma_a, sigma_b,
              x_bar, y_bar, conf, 0))[0][0]
    Tmin = optimize.leastsq(
        FitFuncSIageTW, init_t,
...
def discordance(age_7Pb_5U, age_6Pb_8U, age_7Pb_6Pb, method):
...
def legend_data_number(ax, axn, x, y):
    ...
    ax[axn].text(0.02, 0.98, '$N$ = %d, $n$ = %d' % (N, n_in),
                 transform=ax[axn].transAxes, verticalalignment='top',
                 fontsize=legend_font_size)
...
def plot_data_point(ax, axn, X, Y, ind, outd, outd_disc):
    for i in range(len(X)):
        if (i in ind):
            ax[axn].plot(X[i], Y[i],
                         dp1_marker_type, alpha=dp1_marker_alpha,
            ...
            ax[axn].plot(X[i], Y[i],
                         dp1_marker_type, alpha=dp2_marker_alpha,
            ...
            ax[axn].plot(X[i], Y[i],
                         dp0_marker_type, alpha=dp0_marker_alpha,
...
def plot_2D_wm(ax, axn, X, Y, sigma_X, sigma_Y,
                rho_XY, cr, legend_pos_x, legend_pos_y):
    Xwm_bar, Ywm_bar, MSWDwm, sigma_Xwm_bar, sigma_Ywm_bar, cov_XYwm_bar = twoWM(
        X, Y, sigma_X, sigma_Y, rho_XY, conf=cr)
    twm_ell = myEllipse(0, Xwm_bar, Ywm_bar,
                        sigma_Xwm_bar, sigma_Ywm_bar,
                        cov_XYwm_bar, conf=cr,
                        alpha=twm_ee_alpha, fc=twm_ee_fc,
                        edgecolor=twm_ee_ec, edgewidth=twm_ee_ew)
    ax[axn].add_artist(twm_ell)

    # legend
    ax[axn].text(legend_pos_x, legend_pos_y,
                 '2D weighted mean [%d%% conf.] (MSWD=%s)'
                 % (cr*100, format(MSWDwm,dignum)),
                 transform=ax[axn].transAxes, verticalalignment='top',
                 fontsize=legend_font_size)
...
def concordia_age(ctype, X, Y, sigma_X, sigma_Y, rho_XY, cr):
    if (ctype == 'conv'):
        T_lsq, S_lsq, MSWD, P = ConcAgeConv(X, Y, sigma_X, sigma_Y, rho_XY,
                                         Tinit=age_unit, conf=cr)
        X_lsq, Y_lsq = ConcLineConv(T_lsq)
    elif (ctype == 'tw'):
        T_lsq, S_lsq, MSWD, P = ConcAgeTW(X, Y, sigma_X, sigma_Y, rho_XY,
                                       Tinit=age_unit, conf=cr)
...
def plot_concordia_age(ax, axn, T_lsq, S_lsq, X_lsq, Y_lsq, MSWD, cr,
                       legend_pos_x, legend_pos_y):

    ax[axn].plot(X_lsq, Y_lsq, ca_marker_type, color=ca_marker_fc,
    ...
    ax[axn].text(legend_pos_x, legend_pos_y,
                 ...
                 % (format(T_lsq/age_unit,dignum), format(S_lsq/age_unit,dignum),
                    age_unit_name, cr*100),
                 transform=ax[axn].transAxes, verticalalignment='top',
                 fontsize=legend_font_size)

# ------------------------------------------------
# Text for MSWD of concordia ages
def plot_concordia_age_MSWD(ax, axn, MSWD, ca_mswd, p_value,
                            legend_pos_x, legend_pos_y):

    if (ca_mswd == 1):
        ax[axn].text(legend_pos_x, legend_pos_y,
                     r'(MSWD of equivalence = %s, p($\chi^2$) = %s)' %
                     (format(MSWD,dignum), format(p_value,dignum)),
                     transform=ax[axn].transAxes, verticalalignment='top',
                     fontsize=legend_font_size)
    elif (ca_mswd == 2):
        ax[axn].text(legend_pos_x, legend_pos_y,
...
def plot_concordia_intercept_age(ax, axn, ctype, X, Y,
                                 sigma_X, sigma_Y, cr,
                                 rho_XY, range_XY, T_lsq, case,
                                 legend_pos_x, legend_pos_y):
    ...
    if (ctype == 'conv'):
        Tsi, Tmin, Tmax = SIageConv(
            ai, bi, sigma_a, sigma_b, Xsi_bar, Ysi_bar,
            init_t=T_lsq, conf=cr)
    elif (ctype == 'tw'):
    ...
    ax[axn].fill_between(xx, y1, y2, where=y2 >= y1,
                         facecolor=ia_fill_color, edgecolor='None',
                         alpha=ia_alpha, interpolate=True)
    ax[axn].plot(xx, bi*xx+ai, linewidth=ia_line_width,
                 color=ia_line_color)
...
def plot_oneD_weighted_mean(ax, axn, oneD_age_type,
                            Tall, s1, ind, outd, outd_disc, cr,
                            legend_pos_x, legend_pos_y):
    ...
    ax[axn].plot([0., len(Tall)+1], [Twm, Twm],
    ...
    for i in range(0, len(Tall)):
        if (i in ind):
            ax[axn].errorbar(
                i+1, Tall[i],
                yerr=stats.norm.ppf(cr+(1-cr)/2.)*s1[i],
                ecolor=oneD_bar_color, linewidth=oneD_bar_line_width,
                fmt=dp1_marker_type,
                markersize=dp1_marker_size,
                markerfacecolor=dp1_marker_fc,
                markeredgecolor=dp1_marker_ec,
    ...
    legend_data_number(ax, axn,
                       legend_pos_x[0], legend_pos_y[0])

    ax[axn].text(legend_pos_x[0], legend_pos_y[1],
                 u'Weighted mean = %s ±%s %s [%d%% conf.] (MSWD = %s)' %
                 (format(Twm,dignum), format(sm,dignum), age_unit_name,
                  cr*100, format(MSWD,dignum)),
                 transform=ax[axn].transAxes, verticalalignment='top',
                 fontsize=legend_font_size)

    ax[axn].text(legend_pos_x[0], legend_pos_y[2],
...
def plot_Th_U(axb, Th_U, Th_U_e, Tall, s1, ind, outd, outd_disc, cr,
              range_hist_x, range_hist_y2):
    ...
    for i in range(len(Tall)):
        if (i in ind):
            axb.errorbar(
                Tall[i], Th_U[i],
                xerr=stats.norm.ppf(cr+(1-cr)/2.)*s1[i],
                yerr=Th_U_e[i],
                ecolor=oneD_bar_color, linewidth=oneD_bar_line_width,
                fmt=dp1_marker_type,
                markersize=dp1_marker_size,
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option("-i", "--in", dest="inputfile",
                      help="Name of input data file", metavar="FILE",
    ...
    parser.add_option("-g", "--gui", help="Use GUI",
                      action="store_true", dest="__gui__", default=False)
    parser.add_option("-n", "--no-gui", help="Do not use GUI",
                      action="store_false", dest="__gui__", default=False)
    parser.add_option("-d", "--driver", default="pdf", dest="driver",
                      choices=["qt4agg", "pdf"],
    ...
    if (options.__gui__):
        ...
        [infile, outfile, conffile] = set_filename_input_gui(infile_path,
                                                             options.driver)
    else:
        ...
        if (options.outfile):
            outfile = set_filename_output(options.outfile, options.driver,
                                          options.opt_force_overwrite)
        else:
            outfile = set_filename_output(infile, options.driver,
                                          options.opt_force_overwrite)
    ...
    twm_ee_alpha = config.getfloat('Graph', 'twm_ee_alpha')  # 0.5
    ...
    kde_multiplier = config.getfloat('Graph', 'kde_multiplier')  # number of samples

    # cumulative probability density
    # 1 sigma (68.3), 2 sigma (95.4%), and 3 sigma (99.7%)
    dp_ee_cr = 2.*stats.norm.cdf(dp_ee_sigma)-1.0
    twm_ee_cr = 2.*stats.norm.cdf(twm_ee_sigma)-1.0
    ca_cr = 2.*stats.norm.cdf(ca_sigma)-1.0
    ia_cr = 2.*stats.norm.cdf(ia_sigma)-1.0
    oneD_cr = 2.*stats.norm.cdf(oneD_sigma)-1.0
    Th_U_cr = 2.*stats.norm.cdf(Th_U_sigma)-1.0
    ...
    if (range_automatic_cc):
        range_XY = [[np.exp(l235U*graph_age_min*age_unit) - 1,
    ...
    if (range_automatic_twc):
        range_xy = [[1./range_XY[1][1], 1./range_XY[1][0]],
                    [1./U85r*range_XY[0][0]/range_XY[1][0],
                     1./U85r*range_XY[0][1]/range_XY[1][1]]]
    else:
        range_xy = range_xy
    ...
    dt_name_column = ['7Pb_5U', '7Pb_5U_1s',
                      '6Pb_8U', '6Pb_8U_1s',
                      '7Pb_6Pb', '7Pb_6Pb_1s']

    column_num_isotopic_ratio = [c_7Pb5U, c_7Pb5U_e,
                                 c_6Pb8U, c_6P8U_e,
                                 c_7Pb6Pb, c_7Pb6Pb_e]
    ...
    data = np.loadtxt(infile, delimiter=delim,
    ...
    if (opt_Th_U):
        Th_U = np.loadtxt(infile, delimiter=delim,
                          usecols=Th_U_row_num, skiprows=rows_of_header)
        if (Th_U_inverse):
            Th_U = 1./Th_U
        if (Th_U_error_num):
            Th_U_e = np.loadtxt(infile, delimiter=delim,
                                usecols=Th_U_error_num,
                                skiprows=rows_of_header)
        else:
            Th_U_e = Th_U * 0.0
    ...
    (age_7Pb_6Pb, age_7Pb_6Pb_se_plus,age_7Pb_6Pb_se_minus) = calc_age_7Pb_6Pb(age_unit, y, sigma_y, age_7Pb_6Pb, ca_cr)
    ...
    if debug:
        ...
        for i in range(len(y)):
            print('207Pb/206Pb age = %s +%s -%s %s'
                  % (format(age_7Pb_6Pb[i]/age_unit,dignum),
                     format(age_7Pb_6Pb_se_plus[i]/age_unit,dignum),
                     format(age_7Pb_6Pb_se_minus[i]/age_unit,dignum), age_unit_name))


    disc_percent = discordance(age_7Pb_5U, age_6Pb_8U, age_7Pb_6Pb,
                               method=disc_type)
    ...
    if (error_type):
        ...
        print('column[%d]   column[%d] column[%d]   column[%d] column[%d]    column[%d]'
    ...
    # discordance
    if (opt_exclude_disc):
        print('Discordant data (> %s%%) are excluded from analysis.'
        ...
        print('Discordant data points are')
        for i in outd_disc:
            print('%d %s%%' % (i, format(disc_percent[i],dignum)))
    else:
    ...
    print('Accepted data points are'),  # python2
    # print('Accepted data points are', end=' '),  # python3
    print(ind)
    ...
    if (plot_diagrams[0] == 1):
        ...
        print('--------------------------------------------------')
        print(('Plotting %s: Conventional concordia diagram') % axn_title)
        ...
        if (opt_concordia_age):
            ...
            plot_concordia_age_MSWD(
                ...
                legend_pos_x[legend_pos], legend_pos_y[legend_pos])
            if (ca_mswd==1):
                print(r'    (MSWD of equivalence=%s, p(chi^2)=%s)' %
                      (format(MSWD,dignum), format(Pvalue,dignum)))
            elif (ca_mswd==2):
                print(r'    (MSWD of combined=%s, p(chi^2)=%s)' %
                      (format(MSWD,dignum), format(Pvalue,dignum)))
            else:
                print(r'    (MSWD of concordance=%s, p(chi^2)=%s)' %
                      (format(MSWD,dignum), format(Pvalue,dignum)))
    ...
    if (plot_diagrams[1] == 1):
        ...
        print(('Plotting %s: Tera-Wasserburg concordia diagram') % axn_title)
    ...
    if (plot_diagrams[2] == 1):
        ...
        print(('Plotting %s: One-dimensional bar plot') % axn_title)
        ...
        legend_pos_x, legend_pos_y = calc_legend_pos([[(N+1)*0.05, (N+1)*0.05],
                                                  range_oneD_y])
        T_owm, S_owm, MSWD_owm = plot_oneD_weighted_mean(
            ax, axn, oneD_age_type, Tall, s1,
            ind, outd, outd_disc, oneD_cr,
            legend_pos_x, legend_pos_y)
        print(u'1D weighted mean age = %s ±%s %s [%d%% conf.] (MSDW=%s)'
              % (format(T_owm,dignum), format(S_owm,dignum),
                 age_unit_name, oneD_cr*100, format(MSWD_owm,dignum)))
    ...
    if (plot_diagrams[3] == 1):
        ...
        print(('Plotting %s: Histogram') % axn_title)
        ...
        if (opt_Th_U):
            ...
            axb = ax[axn].twinx()
            plot_Th_U(axb, Th_U, Th_U_e, Tall, s1, ind, outd, outd_disc, oneD_cr,
                      range_hist_x, range_hist_y2)
        ...
        n, bins, rects = ax[axn].hist(
            ...
            hist_bin_num, histtype='barstacked',
            color=(hist_bin_color1, hist_bin_color2, hist_bin_color0),
            alpha=hist_bin_alpha, edgecolor='k', zorder=0, range=ax[axn].get_xlim())
